...
# --- Fragment: Isolated Search Result Card ---
@st.fragment
def render_search_result(idx, vid):
    with st.container(border=True):
        ...
        with info_col:
            ...
            st.caption(
                f":material/account_circle: {vid.get('uploader', 'Unknown')}  •  "
                f":material/schedule: {vid.get('duration_string', '?')}  •  "
                f":material/visibility: {vid.get('views', 0):,}"
            )
            ...
            if ctrl_dl.button(" ", icon=":material/download:", key=f"yt_dl_{idx}", help="Download", width="stretch"):
                status_msg = st.empty()
                ...
                def _search_dl_task(v_url, v_title, audio_flag, res, status_placeholder):
                    ...
                    if success:
                        status_placeholder.success(f":material/check_circle: {v_title} downloaded successfully!")
...
st.header(":material/smart_display: YouTube Downloader & Search")
...
with st.container(border=True):
    new_dest = st.text_input(
        ":material/save: Save Downloads To:",
...
tab1, tab2 = st.tabs([":material/search: Search & Download", ":material/link: Direct URL / Playlist Downloader"])
...
with tab1:
    ...
    if st.session_state.yt_search_results:
        ...
        for idx, vid in enumerate(st.session_state.yt_search_results):
            # We now call our fragment. Interacting with widgets inside
            # will NOT rerun the entire page or loop.
            render_search_result(idx, vid)
...
with tab2:
    ...
    if st.button("Start Download", icon=":material/rocket_launch:", type="primary", width="stretch", key="yt_direct_dl_btn"):
        if not direct_url:
