...
if col_btn.button("Refresh List", type="secondary", width="stretch", icon=":material/refresh:"):
    st.rerun()
            ...
            for c in containers:
                with st.container(border=True):
                    col_info, col_status, col_actions = st.columns(
                        [3, 1, 2], vertical_alignment="center")
                    ...
                    with col_status:
                        if c['status'] == 'running':
                            ...
                            st.warning(
                                f"🟡 {c['status'].capitalize()}", icon=None)
                    ...
                    with col_actions:
                        ...
                        if act1.button("Start", key=f"start_{c['id']}", type="primary", disabled=is_running, width="stretch"):
                            act_succ, act_msg = container_action(
                                c['id'], "start")
                            if act_succ:
                                st.toast(act_msg)
                            else:
