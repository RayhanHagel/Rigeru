...
if "media_vid_out" not in st.session_state:
    st.session_state.media_vid_out = os.path.join(
        os.path.expanduser('~'), 'Videos')
if "media_img_in" not in st.session_state:
    st.session_state.media_img_in = ""
if "media_img_out" not in st.session_state:
    st.session_state.media_img_out = os.path.join(
        os.path.expanduser('~'), 'Pictures')

st.header("🎞️ Visual Media Compressor")
st.markdown(
    "Locally trim videos and batch-compress images to save hard drive space.")

tab1, tab2 = st.tabs(["🎥 Video Trimmer & Compressor",
...
with tab1:
    with st.container(border=True):
        ...
        st.session_state.media_vid_out = st.session_state.get(
            "vid_out_display", st.session_state.media_vid_out)

    st.subheader("Trimming & Profile")
    col_t1, col_t2 = st.columns(2)
    start_t = col_t1.number_input(
        "Start Time (seconds)", min_value=0.0, value=0.0, step=1.0, key="vid_start")
    end_t = col_t2.number_input(
        "End Time (seconds)", min_value=0.0, value=99999.0, step=1.0, key="vid_end")

    # Handbrake Presets Logic
    profile = st.selectbox(
        "Compression Profile",
        ["Custom Configuration", "Handbrake: Fast 1080p",
            "Handbrake: High Quality 1080p", "Handbrake: Web Optimized 720p"]
    )

    if profile == "Custom Configuration":
        target_res = st.selectbox(
            "Max Resolution", ["Keep Original", "1080p", "720p", "480p"], index=1)
        col_crf, col_preset = st.columns(2)
        crf_val = col_crf.slider(
            "CRF Quality (Lower = Better/Larger File)", 0, 51, 23)
        enc_preset = col_preset.selectbox("Encoding Speed Preset", [
                                          "ultrafast", "superfast", "veryfast", "faster", "fast", "medium", "slow", "slower", "veryslow"], index=4)

        col_a1, col_a2 = st.columns(2)
        keep_all_audio = col_a1.checkbox(
            "Keep All Audio Tracks (Multi-track)", value=True)
        audio_codec = col_a2.selectbox("Audio Codec", [
                                       "aac", "copy"], help="AAC recompresses audio, Copy keeps original quality.")
    else:
        ...
        st.info(
            f"**Loaded Preset:** Max Res: {target_res} | CRF: {crf_val} | Speed: {enc_preset} | Audio: {audio_codec.upper()}")
...
with tab2:
    with st.container(border=True):
        ...
        st.session_state.media_img_in = st.session_state.get(
            "img_in_display", st.session_state.media_img_in)
        ...
        st.session_state.media_img_out = st.session_state.get(
            "img_out_display", st.session_state.media_img_out)

    st.subheader("Compression Settings")
    img_quality = st.slider("JPEG Quality", min_value=10,
                            max_value=100, value=80, key="img_quality")

    col_w, col_h = st.columns(2)
    img_width = col_w.number_input(
        "Target Box Width (px)", min_value=0, value=1920, help="Set to 0 to ignore")
    img_height = col_h.number_input(
        "Target Box Height (px)", min_value=0, value=1080, help="Set to 0 to ignore")

    fit_mode = st.selectbox(
        "Resizing Mode (If dimensions provided)",
        ["Maintain Aspect Ratio (Fit Inside)", "Stretch to Fit", "Pad with Black Bars",
         "Pad with White Bars", "Pad with Blurred Background"]
    )
