...
def guess_application_from_path(folder_path: str) -> str:
    ...
    if "scoop" in folder_path.lower(): return f"📦 Scoop App ({folder_name})"
    if "windowsapps" in folder_path.lower(): return "🪟 Windows Store App"
    if "python" in folder_name: return "🐍 Python Environment"
    if "node" in folder_name or "npm" in folder_name: return "🟢 Node.js / NPM"
    if "git" in folder_name: return "🔀 Git Version Control"
    ...
    return "📁 System / Unknown Utility"

@st.cache_data(ttl=60)
def get_all_paths() -> tuple[list[dict], str, str]:
    """Fetches and parses both System and User PATH variables."""
    # System PATH
    sys_path_raw = get_registry_value(winreg.HKEY_LOCAL_MACHINE, r"System\CurrentControlSet\Control\Session Manager\Environment", "Path")
    # User PATH
    user_path_raw = get_registry_value(winreg.HKEY_CURRENT_USER, r"Environment", "Path")
