...
export function DataNode({ id, data, selected }: any) {
    ...
    <div className={`group px-4 py-3 shadow-lg rounded-xl border-2 bg-[var(--theme-ui-bg)] backdrop-blur-md min-w-[150px] transition-all duration-200 ${selected ? 'border-purple-500 shadow-[0_0_15px_rgba(168,85,247,0.4)]' : 'border-[var(--theme-ui-border)]'}`}>
      ...
      <div className="flex items-center gap-2">
        ...
        <div className="flex flex-col">
          <div className="text-sm font-bold text-zinc-100">{data.label || 'Data Source'}</div>
          <div className="text-xs text-zinc-400">{data.fileName || 'No file selected'}</div>
          {data.targetCol && <div className="text-xs text-purple-300">Target: {data.targetCol}</div>}
        </div>
      </div>
