...
export default function FisheyePage() {
  ...
  const handleProcess = async () => {
    ...
    try {
      ...
      const res = await fetch("/api/media-vision/fisheye/batch", {
        method: "POST",
        body: batchData,
      });
  ...
  return (
    <div className="w-full h-full p-6 lg:p-10 relative z-10 overflow-y-auto animate-slide-up flex flex-col font-sans">
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-4 mb-6 border-b border-primary/30 pb-4 shrink-0">
        <div>
          <h1 className="text-3xl font-bold text-white tracking-tight">Fisheye Distortion</h1>
          <p className="text-zinc-400 text-sm font-medium">Apply a fisheye lens effect to your images.</p>
        </div>
          ...
          <SectionHeader title="Upload media" />
            ...
            <div className="flex flex-col gap-2">
              <div className="flex items-center justify-between">
                <label className="text-sm font-medium text-white">Distortion Strength</label>
                <span className="text-xs font-mono text-zinc-400 bg-zinc-950 px-2 py-1 rounded-md border border-white/5">
                  {strength.toFixed(2)}
                </span>
              </div>
              <input
                ...
                className="w-full accent-primary bg-zinc-950 h-2 rounded-lg appearance-none cursor-pointer"
              />
            </div>
