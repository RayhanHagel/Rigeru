...
export default function FisheyePage() {
  ...
  const handleProcess = async () => {
    ...
    try {
      ...
      batchData.append("strength", strength.toString());

      // Direct call to backend — bypasses Next.js proxy to avoid timeout on long ops
      const token = localStorage.getItem("auth_token") || "";
      const baseUrl = window.location.protocol + "//" + window.location.hostname + ":8000";

      const res = await fetch(`${baseUrl}/api/media-vision/fisheye/batch`, {
        method: "POST",
  ...
  return (
    <div className="w-full h-full p-6 lg:p-10 relative z-10 overflow-y-auto animate-slide-up flex flex-col font-sans">
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-4 mb-6 border-b border-[var(--theme-ui-border)] pb-4 shrink-0">
        <div>
          <h1 className="text-3xl font-bold text-[var(--theme-heading)] tracking-tight">Fisheye Distortion</h1>
          <p className="text-[var(--theme-text)] text-sm font-medium">Apply a fisheye lens effect to your images.</p>
        </div>
                ...
                <ModernTabContent activeTab="batch">
                  <div className="flex flex-col gap-2">
                    ...
                    <BatchFolderSelector 
                      ...
                      subtitle="Batch process multiple images"
                    />
                  </div>
                </ModernTabContent>
              )}
              ...
              <div className="grid grid-cols-1 gap-6 mt-4">
                ...
                <div className="p-5 rounded-xl space-y-5 shadow-sm border border-[var(--theme-ui-border)] bg-[var(--theme-ui-bg)] backdrop-blur-md">
                  <div className="flex items-center gap-2 font-medium pb-2 border-b" style={{ borderColor: "color-mix(in srgb, var(--theme-heading) 20%, transparent)" }}>
                    <h3 className="text-[var(--theme-heading)]">Lens Adjustments</h3>
                  </div>

                  <div className="space-y-4">
                    <div className="space-y-1.5">
                      <div className="flex items-center justify-between">
                        <label className="text-sm font-medium text-[var(--theme-heading)]">Distortion Strength</label>
              ...
              {processedUrls.length === 0 ? (
                ...
                <div className="w-full h-full flex flex-col gap-4">
                  <div className="flex items-center justify-between mb-2">
                    <Button variant="ghost" size="sm" onClick={() => setSelectedBatchIndex(null)} icon={<Icon name="arrow_back" size={16} />}>
                      Back to Grid
                    </Button>
                    <div className="flex items-center gap-2">
                      <Button variant="secondary" size="sm" onClick={() => setSelectedBatchIndex(Math.max(0, selectedBatchIndex - 1))} disabled={selectedBatchIndex === 0} icon={<Icon name="chevron_left" size={16} />}>Prev</Button>
                      <span className="text-sm font-medium text-[var(--theme-heading)]">{selectedBatchIndex + 1} / {processedUrls.length}</span>
                      <Button variant="secondary" size="sm" onClick={() => setSelectedBatchIndex(Math.min(processedUrls.length - 1, selectedBatchIndex + 1))} disabled={selectedBatchIndex === processedUrls.length - 1} icon={<Icon name="chevron_right" size={16} />}>Next</Button>
                    </div>
                  </div>
                  <ImageCompareSlider
                    ...
                    {processedUrls.map((url, i) => (
                      ...
                      >
                        ...
                        <div className="absolute inset-0 bg-black/50 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center">
                          <Icon name="compare" size={24} className="text-white drop-shadow-md" />
                        </div>
