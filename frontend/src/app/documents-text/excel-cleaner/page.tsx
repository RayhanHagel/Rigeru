...
export default function ExcelCleanerPage() {
  ...
  const fetchPreview = async (info: { hash_name: string; original_name: string; file_type: string }, header: boolean) => {
    setIsLoading(true);
    setErrorMsg("");
    
    const formData = new FormData();
    formData.append("file_hash", info.hash_name);
    formData.append("has_header", header.toString());
    
    try {
      const res = await fetch("/api/files-documents/excel-cleaner/preview", {
        method: "POST",
        body: formData,
      });
  ...
  const handleHeaderChange = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (fileInfo) {
      await fetchPreview(fileInfo, checked);
  ...
  const handleApplyCleaning = async () => {
    ...
    try {
      const res = await fetch("/api/files-documents/excel-cleaner/process", {
        method: "POST",
        body: formData,
      });
  ...
  const handleDownload = async (format: "CSV" | "Excel") => {
    ...
    try {
      ...
      a.href = url;
      const baseName = fileInfo.original_name.substring(0, fileInfo.original_name.lastIndexOf('.')) || fileInfo.original_name;
