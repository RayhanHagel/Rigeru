...
export default function FileTimestampsPage() {
  ...
  const handleReadTimestamps = async () => {
    ...
    try {
      const res = await fetch("http://127.0.0.1:8000/api/subtitles/timestamps/read", {
        method: "POST",
  ...
  const handleSaveTimestamps = async () => {
    ...
    try {
      const res = await fetch("http://127.0.0.1:8000/api/subtitles/timestamps/save", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ 
          file_path: filePath,
  ...
  return (
    <div className="p-6 max-w-5xl mx-auto space-y-8 animate-in fade-in">
      <div>
        <STHeader title="🕒 Timestamp Modifier" />
        <p className="text-zinc-400 mt-2">
          Forcefully rewrite OS-level file creation, modification, and access timestamps for any file.
        </p>
      </div>
      ...
      <STContainer title="Edit File Timestamps" icon={<Clock className="text-indigo-400" size={20} />}>
        <div className="space-y-6">
          <div className="space-y-1.5">
            ...
            <input 
              ...
              className="w-full bg-zinc-900 border border-zinc-800 rounded-lg p-3 text-white focus:outline-none focus:border-indigo-500 transition-colors [color-scheme:dark]"
            />
          </div>
