...
export default function SubtitleFetcherPage() {
  ...
  const handleSearch = async () => {
    ...
    try {
      const res = await fetch("http://127.0.0.1:8000/api/subtitles/fetcher/search", {
        method: "POST",
        body: formData
      });
  ...
  const handleDownload = async (fileId: string) => {
    ...
    try {
      const res = await fetch("http://127.0.0.1:8000/api/subtitles/fetcher/download", {
        method: "POST",
        body: formData
      });
  ...
  return (
    <div className="p-6 max-w-5xl mx-auto space-y-8 animate-in fade-in">
      <div>
        <STHeader title="🔤 Local Subtitle Fetcher" />
        <p className="text-zinc-400 mt-2">
          Find the <i>exact</i> subtitle for your video. This tool analyzes your local video to generate a tiny digital fingerprint, keeping your media completely private.
        </p>
      </div>

      <STContainer title="OpenSubtitles Configuration" icon={<Settings className="text-indigo-400" size={20} />}>
        <div className="space-y-4">
          <p className="text-sm text-zinc-300">
            You need a free REST API Key from <a href="https://opensubtitles.com/" target="_blank" rel="noreferrer" className="text-indigo-400 hover:underline">OpenSubtitles.com</a>.
          </p>
          <div className="flex items-center gap-3">
            <div className="relative flex-1">
              ...
              <input 
                ...
                className="w-full bg-zinc-900 border border-zinc-800 rounded-lg py-2 pl-10 pr-4 text-white focus:outline-none focus:border-indigo-500 transition-colors"
              />
            </div>
      ...
      <STContainer title="Search Subtitles" icon={<Search className="text-indigo-400" size={20} />}>
