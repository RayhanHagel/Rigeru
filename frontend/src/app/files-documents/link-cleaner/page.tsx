...
export default function LinkCleanerPage() {
  ...
  const handleAnalyze = async () => {
    ...
    try {
      const res = await fetch("http://127.0.0.1:8000/api/files-documents/mega-cleaner/process", {
        method: "POST",
        headers: {
    ...
    <div className="flex flex-col gap-6 p-6 max-w-5xl mx-auto w-full h-full">
      <div className="flex flex-col gap-2">
        <h1 className="text-3xl font-bold text-white tracking-tight flex items-center gap-3">
          <FolderArchive className="text-red-500" size={32} />
          Link Cleaner
        </h1>
        <p className="text-zinc-400 text-sm font-medium">
          Extract individual file links from master MEGA folders, filter by size, and remove duplicates automatically.
        </p>
      </div>
      ...
      {results && (
        <div className="flex flex-col gap-6 animate-fade-in">
          ...
          {/* Aggregated Output Tabs */}
          <div className="bg-zinc-900/50 border border-white/10 rounded-2xl overflow-hidden backdrop-blur-sm flex flex-col h-[500px]">
            <div className="flex border-b border-white/10 bg-zinc-900/80">
              <button
                onClick={() => setActiveTab("raw")}
                className={`flex-1 py-3 px-4 text-sm font-medium flex items-center justify-center gap-2 transition-colors ${
                  activeTab === "raw" ? "text-white border-b-2 border-red-500" : "text-zinc-500 hover:text-zinc-300"
                }`}
              >
                <List size={16} />
                Raw Links
              ...
              <button
                ...
                className={`flex-1 py-3 px-4 text-sm font-medium flex items-center justify-center gap-2 transition-colors ${
                  activeTab === "named" ? "text-white border-b-2 border-red-500" : "text-zinc-500 hover:text-zinc-300"
                }`}
              >
                <FileText size={16} />
                With Names
              ...
              <button
                ...
                className={`flex-1 py-3 px-4 text-sm font-medium flex items-center justify-center gap-2 transition-colors ${
                  activeTab === "logs" ? "text-white border-b-2 border-red-500" : "text-zinc-500 hover:text-zinc-300"
                }`}
              >
