...
export default function FileOrganizerPage() {
  ...
  const handleScan = async () => {
    ...
    setIsLoading(true);
    try {
      const res = await fetch('http://127.0.0.1:8000/api/files-documents/file-organizer/scan', {
        method: 'POST',
  ...
  const handleOpen = async () => {
    try {
      await fetch('http://127.0.0.1:8000/api/files-documents/file-organizer/open', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
  ...
  const handleDelete = async () => {
    setIsLoading(true);
    try {
      const res = await fetch('http://127.0.0.1:8000/api/files-documents/file-organizer/action', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          action: 'delete',
  ...
  const handleUndo = async () => {
    ...
    setIsLoading(true);
    try {
      const res = await fetch('http://127.0.0.1:8000/api/files-documents/file-organizer/undo', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
      ...
      <div className="max-w-5xl mx-auto p-4 md:p-6 space-y-6">
        <div className="flex flex-col mb-6">
          ...
          <p className="text-zinc-400 mt-2">
            Browse your folders, scan, then <strong className="text-zinc-200">Open</strong>, <strong className="text-zinc-200">Move</strong>, <strong className="text-zinc-200">Rename</strong>, <strong className="text-zinc-200">Skip</strong>, <strong className="text-zinc-200">Delete</strong>, or <strong className="text-zinc-200">Undo</strong>.
          </p>
        </div>
            ...
            <div className="space-y-2">
              ...
              <div className="relative">
                ...
                <input
                  ...
                  className="w-full bg-zinc-950 border border-white/10 rounded-xl pl-10 pr-12 py-3 text-zinc-200 focus:outline-none focus:ring-2 focus:ring-blue-500/50"
        ...
        {filesList.length > 0 && !isFinished && (
          <div className="bg-zinc-900/40 border border-white/5 rounded-2xl p-6 shadow-xl animate-fade-in flex flex-col md:flex-row gap-8">

            {/* Left: Preview */}
            <div className="w-full md:w-1/2 flex flex-col items-center justify-center bg-black/40 rounded-xl p-4 border border-white/5 min-h-[300px] overflow-hidden">
              {currentFile && (
                // eslint-disable-next-line @next/next/no-img-element
                <img
                  src={`http://127.0.0.1:8000/api/files-documents/file-organizer/preview?path=${encodeURIComponent(srcFilePath)}`}
                  alt="Preview"
            ...
            <div className="w-full md:w-1/2 flex flex-col justify-between space-y-6">
              ...
              <div className="space-y-4 pt-4 border-t border-white/10">
                <h3 className="text-lg font-semibold text-zinc-200 flex items-center gap-2">
                  <FolderOutput size={20} className="text-green-400" />
                  Move & Rename
                </h3>
        ...
        {isFinished && (
          <div className="bg-zinc-900/40 border border-white/5 rounded-2xl p-12 shadow-xl flex flex-col items-center justify-center text-center animate-fade-in">
        ...
        {/* Undo Section */}
        {history.length > 0 && (
          <div className="flex justify-end animate-fade-in pb-8">
            <Button
              variant="secondary"
              onClick={handleUndo}
              disabled={isLoading}
