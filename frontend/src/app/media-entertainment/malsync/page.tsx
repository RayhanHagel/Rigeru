...
import { Search, Library, RefreshCw, LogIn, MonitorPlay, BookOpen, Settings2, Trash2, CheckCircle2, AlertCircle, Plus, Minus, Save, Link2 } from "lucide-react";
...
function MalSyncContent() {
  ...
  const fetchAuthStatus = async () => {
    try {
      const res = await fetch("http://127.0.0.1:8000/api/media-entertainment/malsync/auth/status");
  ...
  const fetchLibrary = async () => {
    try {
      const res = await fetch(`http://127.0.0.1:8000/api/media-entertainment/malsync/library/${mediaType}`);
      const data = await res.json();
      setLibrary(data);
    } catch (e) {
      console.error(e);
  ...
  const fetchCredentials = async () => {
    try {
      const res = await fetch("http://127.0.0.1:8000/api/media-entertainment/malsync/credentials");
      const data = await res.json();
  ...
  useEffect(() => {
    if (authCode) {
      ...
      fetch("http://127.0.0.1:8000/api/media-entertainment/malsync/auth/exchange", {
        method: "POST",
      ...
      .then(res => res.json())
      .then(data => {
        router.replace("/media-entertainment/malsync"); // Clear URL
        setIsLoggedIn(true);
      })
      .catch(console.error)
  ...
  const handleSync = async () => {
    ...
    setSyncMessage("");
    try {
      const res = await fetch("http://127.0.0.1:8000/api/media-entertainment/malsync/sync", {
        method: "POST"
      });
      const data = await res.json();
      if (res.ok) {
  ...
  const handleLogin = async () => {
    try {
      const res = await fetch("http://127.0.0.1:8000/api/media-entertainment/malsync/auth/url");
      const data = await res.json();
      if (data.url) {
  ...
  const saveCredentials = async () => {
    ...
    try {
      await fetch("http://127.0.0.1:8000/api/media-entertainment/malsync/credentials", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
  ...
  const updateProgress = async (malId: string, currentProg: number, delta: number) => {
    ...
    try {
      await fetch(`http://127.0.0.1:8000/api/media-entertainment/malsync/progress/${mediaType}`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
  ...
  const removeItem = async (malId: string) => {
    try {
      await fetch(`http://127.0.0.1:8000/api/media-entertainment/malsync/library/${mediaType}/${malId}`, {
        method: "DELETE"
      });
      fetchLibrary(); // Refresh
    } catch (e) {
  ...
  return (
    <div className="w-full h-full flex flex-col overflow-y-auto p-10 animate-fade-in relative z-10">
      <div className="mb-8">
        <h1 className="text-4xl font-bold mb-2 flex items-center gap-3">
          <Library size={36} /> Local MAL Tracker
        </h1>
        <p className="text-zinc-400 text-lg">Track your watching and reading progress locally and sync directly with your MAL account.</p>
      </div>
      ...
      <div className="flex border-b border-white/10 mb-8">
        <button 
          ...
          className={`px-6 py-3 font-medium text-sm border-b-2 flex items-center gap-2 ${activeTab === "library" ? "border-purple-500 text-purple-400" : "border-transparent text-zinc-400 hover:text-zinc-200"}`}
        >
          <Library size={18} /> My Library
        </button>
        <button 
          onClick={() => setActiveTab("settings")}
          className={`px-6 py-3 font-medium text-sm border-b-2 flex items-center gap-2 ${activeTab === "settings" ? "border-purple-500 text-purple-400" : "border-transparent text-zinc-400 hover:text-zinc-200"}`}
        >
          <Settings2 size={18} /> Settings & Sync
        </button>
      </div>

      {isAuthLoading && (
        <div className="mb-6 p-4 bg-purple-500/10 border border-purple-500/20 rounded-xl flex items-center gap-3 text-purple-300">
        ...
        <div className="flex flex-col gap-6">
          ...
          <div className="flex flex-col lg:flex-row gap-4 p-4 bg-zinc-900/50 rounded-xl border border-white/5">
            <div className="flex bg-zinc-900 rounded-md p-1 border border-white/10 shrink-0">
              <button 
                className={`px-6 py-1.5 text-sm rounded-sm transition-colors flex items-center gap-2 ${mediaType === "anime" ? "bg-purple-500/20 text-purple-400 font-medium" : "text-zinc-400 hover:text-white"}`}
                onClick={() => { setMediaType("anime"); setFilterStatus("All Titles"); }}
              >
                <MonitorPlay size={16} /> Anime
              </button>
              <button 
                className={`px-6 py-1.5 text-sm rounded-sm transition-colors flex items-center gap-2 ${mediaType === "manga" ? "bg-purple-500/20 text-purple-400 font-medium" : "text-zinc-400 hover:text-white"}`}
            ...
            <div className="flex-1 relative">
              ...
              <input
                ...
                className="w-full bg-zinc-900 border border-white/10 rounded-md pl-10 pr-4 py-2 text-white focus:border-purple-500 outline-none text-sm h-full"
              />
            </div>
            
            <div className="flex gap-2 shrink-0">
              <select value={filterStatus} onChange={(e) => setFilterStatus(e.target.value)} className="bg-zinc-900 border border-white/10 rounded-md px-3 py-2 text-white outline-none text-sm focus:border-purple-500">
                {filterOptions.map(opt => <option key={opt} value={opt}>{opt}</option>)}
              </select>
              <select value={sortBy} onChange={(e) => setSortBy(e.target.value)} className="bg-zinc-900 border border-white/10 rounded-md px-3 py-2 text-white outline-none text-sm focus:border-purple-500">
          ...
          {Object.keys(library).length === 0 ? (
            <div className="bg-zinc-900/50 border border-blue-500/30 text-blue-300 p-4 rounded-md flex items-start gap-3 mt-4">
                ...
                return (
                  <div key={malId} className="flex flex-col border border-white/10 rounded-lg overflow-hidden bg-zinc-900/30 shadow-md group relative">
                    ...
                    <div className="p-4 flex flex-col flex-1">
                      <a href={item.url} target="_blank" rel="noopener noreferrer" className="font-semibold text-white hover:text-purple-400 line-clamp-2 min-h-[40px] text-center" title={item.title}>
                        {item.title}
                      </a>
      ...
      {activeTab === "settings" && (
        <div className="flex flex-col lg:flex-row gap-8 items-start">
          ...
          <div className="w-full lg:w-1/2 bg-zinc-900/40 border border-white/5 rounded-xl p-8">
            <h2 className="text-2xl font-bold mb-4 flex items-center gap-2"><RefreshCw size={24} /> Sync via OAuth</h2>
            {!clientId ? (
          ...
          {/* Settings Section */}
          <div className="w-full lg:w-1/2 bg-zinc-900/40 border border-white/5 rounded-xl p-8">
            <h2 className="text-2xl font-bold mb-4 flex items-center gap-2"><Settings2 size={24} /> API Configuration</h2>
            <div className="mb-6 text-zinc-300 leading-relaxed">
              <p>To connect to MyAnimeList, you need an API Client.</p>
              <ol className="list-decimal pl-5 mt-2 space-y-1 text-sm">
                <li>Go to <a href="https://myanimelist.net/apiconfig" target="_blank" rel="noopener noreferrer" className="text-purple-400 hover:underline">MAL API Settings</a></li>
                <li>Create a New Client (App Type: Web)</li>
                <li>Set Redirect URL to <code className="bg-zinc-900 px-1.5 py-0.5 rounded text-sm text-purple-400">http://localhost:3000/media-entertainment/malsync</code></li>
              </ol>
            </div>
            ...
            <div className="bg-zinc-900/50 border border-white/10 rounded-lg p-6">
              <div className="mb-4">
                ...
                <input 
                  ...
                  className="w-full bg-zinc-900 border border-white/10 rounded-md px-3 py-2 text-white focus:border-purple-500 outline-none" 
                />
              </div>
              <div className="mb-6">
