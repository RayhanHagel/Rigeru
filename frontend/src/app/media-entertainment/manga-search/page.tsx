...
export default function MangaSearch() {
  ...
  const handleSearch = async () => {
    ...
    setResults({});
    try {
      const res = await fetch(`http://127.0.0.1:8000/api/media-entertainment/manga-search/query?title=${encodeURIComponent(searchQuery)}&websites=${encodeURIComponent(websites.join(","))}`);
  ...
  const handleAdd = async (title: string, urlStr: string) => {
    ...
    try {
      const res = await fetch("http://127.0.0.1:8000/api/media-entertainment/manga-search/add", {
        method: "POST",
  ...
  return (
    <div className="w-full h-full p-6 lg:p-10 animate-fade-in relative z-10 max-w-5xl mx-auto overflow-y-auto">
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-4 mb-10 border-b border-purple-500/30 pb-6">
        <div className="flex items-center gap-4">
          ...
          <div>
            ...
            <p className="text-zinc-400 text-sm font-medium mt-1">Find and add new manga to your library.</p>
          </div>
        </div>
        <Button variant="secondary" icon={<BookOpen size={16} />} onClick={() => router.push('/media-entertainment/manga-library')}>
          Back to Library
        </Button>
      </div>
          ...
          <div>
            ...
            <div className="flex gap-3">
              <input 
                ...
                className="flex-1 bg-zinc-950 border border-white/10 rounded-xl p-3 text-white focus:border-purple-500 outline-none transition-all"
              />
              <Button variant="primary" icon={<Search size={16} />} onClick={handleSearch} isLoading={loading} className="bg-purple-500 hover:bg-purple-600 text-white border-none px-6">
                Search
              </Button>
            </div>
          </div>
      ...
      <div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          {Object.entries(results).map(([title, urlStr]) => {
            ...
            return (
              <div key={title} className="bg-zinc-900/30 border border-white/10 rounded-xl overflow-hidden flex flex-col group hover:border-purple-500/50 transition-all">
                {coverUrl && (
