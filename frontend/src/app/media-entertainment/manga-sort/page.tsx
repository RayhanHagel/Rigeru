...
function SortableItem({ id, item }: { id: string, item: MangaData }) {
  ...
  const coverUrl = item.local_image ? `http://127.0.0.1:8000/static/${item.local_image.split('static/')[1]}` : item.image;

  return (
    <div 
      ref={setNodeRef} 
      style={style} 
      className={`bg-zinc-900/80 border rounded-xl overflow-hidden flex flex-col transition-shadow ${isDragging ? 'border-purple-500 shadow-[0_0_15px_rgba(168,85,247,0.4)] opacity-80' : 'border-white/5 shadow-md'}`}
    >
      <div 
        {...attributes} 
        {...listeners}
      ...
      >
        ...
        <GripHorizontal size={14} className="text-zinc-600 group-hover:text-purple-400" />
      </div>
      ...
      <div className="p-3 flex gap-3 flex-1">
        ...
        <div className="flex flex-col justify-center">
           ...
           <div className="text-xs text-purple-400 bg-purple-500/10 px-2 py-1 rounded-md w-fit">
...
export default function MangaSortPage() {
  ...
  useEffect(() => {
    fetch("http://127.0.0.1:8000/api/media-entertainment/manga-library")
      .then(res => res.json())
  ...
  const handleSave = async () => {
    setIsSaving(true);
    try {
      const res = await fetch("http://127.0.0.1:8000/api/media-entertainment/manga-library/sort", {
        method: "POST",
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-8">
        <div className="flex items-center gap-4">
          ...
          <div>
            ...
            <p className="text-zinc-400 text-sm mt-1">Drag and drop manga cards to reorder your library.</p>
          </div>
        </div>
        
        <div className="flex items-center gap-3">
          <Button variant="secondary" onClick={() => router.push("/media-entertainment/manga-library")} icon={<ArrowLeft size={16} />}>
            Back to Library
          </Button>
