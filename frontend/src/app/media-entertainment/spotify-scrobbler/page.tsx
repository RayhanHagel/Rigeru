...
export default function SpotifyScrobbler() {
  ...
  const fetchFeed = async () => {
    ...
    try {
      ...
      const url = `http://127.0.0.1:8000/api/media-entertainment/spotify-scrobbler/feed?username=${encodeURIComponent(username)}&fetch_method=${encodeURIComponent(fetch_method)}&api_key=${encodeURIComponent(api_key)}&limit=${track_limit}&tz_str=${encodeURIComponent(timezone)}`;
  ...
  return (
    <div className="w-full h-full p-6 lg:p-10 animate-fade-in relative z-10 overflow-y-auto">
      {/* Header */}
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-4 mb-10 border-b border-purple-500/30 pb-4">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-zinc-400 text-sm font-medium mt-1">Live, auto-refreshing feed of your Last.fm</p>
          </div>
        </div>
      ...
      {!spotifyConfig.username ? (
        <div className="flex flex-col items-center justify-center h-64 text-center rounded-3xl border border-dashed border-zinc-800 bg-zinc-900/30 backdrop-blur-sm">
          ...
          <p className="text-zinc-500 text-sm mt-2">Open the Page Settings sidebar on the right to configure.</p>
        </div>
      ) : error ? (
        ...
        <div className="max-w-4xl mx-auto pb-20 animate-slide-up">
          ...
          <div className="flex items-center gap-4 py-3 mb-8">
            <img 
              src={feed.avatar_url?.startsWith('/app/static') ? `http://127.0.0.1:8000${feed.avatar_url.replace('/app/static', '/static')}` : feed.avatar_url!} 
              alt="Avatar" 
